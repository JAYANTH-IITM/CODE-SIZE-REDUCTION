...
class Generator():
    ...
    def  __zcmp32_instr__(self, op=None, val=None):
        cond_str = ''
        if op[0] in ['x1', 'x8', 'x9', 'x18']:
            if val[0] in [16, 32, 48, 64]:
                cond_str += op[-1]+', '
                cond_str += val[-1]
            else:
                new_val = random.choice([16, 32, 48, 64])
                # Update val
                val[0] = new_val
                cond_str += op[-1]+', '
                cond_str += val[-1]
        elif op[0] in ['x19', 'x20', 'x21', 'x22']:
            if val[0] in [32, 48, 64, 80]:
                cond_str += op[-1]+', '
                cond_str += val[-1]
            else:
                # Choose a random value from the allowed list
                new_val = random.choice([32, 48, 64, 80])
                # Update val
                val[0] = new_val        
                cond_str += op[-1]+', '
                cond_str += val[-1]
        elif op[0] in ['x23', 'x24', 'x25']:
            if val[0] in [48, 64, 80, 96]:
                cond_str += op[-1]+', '
                cond_str += val[-1]    
            else:
                # Choose a random value from the allowed list
            ...
            if val[0] in [64, 80, 96, 112]:
                new_val = random.choice([112])
                val[0] = new_val
                cond_str += op[-1]+', '
                cond_str += val[-1]
            else:
        ...
        print(instr)
        return instr
    def  __zcmp64_instr__(self, op=None, val=None):
        
        cond_str = ''
        if op[0] in ['x1', 'x8']:
            if op[0] == 'x1':
                new_val = random.choice([16])
                val[0] = new_val
                cond_str += op[-1]+', '
                cond_str += val[-1]
            elif op[0] == 'x8':
                new_val = random.choice([32])
                val[0] = new_val
                cond_str += op[-1]+', '
                cond_str += val[-1]
        elif op[0] in ['x9' ,'x18']:
            if op[0] == 'x9':
                new_val = random.choice([48])
                val[0] = new_val
                cond_str += op[-1]+', '
                cond_str += val[-1]
            elif op[0] == 'x18':
                new_val = random.choice([64])
                val[0] = new_val
                cond_str += op[-1]+', '
                cond_str += val[-1]
        elif op[0] in ['x19','x20']:
            if op[0] == 'x19':
                new_val = random.choice([80])
                val[0] = new_val
                cond_str += op[-1]+', '
                cond_str += val[-1]
            elif op[0] == 'x20':
                new_val = random.choice([96])
                val[0] = new_val
                cond_str += op[-1]+', '
                cond_str += val[-1]
        elif op[0] in ['x21','x22']:
            if op[0] == 'x21':
                new_val = random.choice([64,80,96])
                val[0] = new_val
                cond_str += op[-1]+', '
                cond_str += val[-1]
            elif op[0] == 'x22':
                ...
                cond_str += op[-1]+', '
                cond_str += val[-1]
        elif op[0] in ['x23','x24']:
            if op[0] == 'x23':
                new_val = random.choice([80,96,112])
                val[0] = new_val
                cond_str += op[-1]+', '
                cond_str += val[-1]
            elif op[0] == 'x22':
                new_val = random.choice([128])
                val[0] = new_val
                cond_str += op[-1]+', '
                cond_str += val[-1]
        elif op[0] == 'x25':
                new_val = random.choice([144])
                val[0] = new_val
                cond_str += op[-1]+', '
                cond_str += val[-1]
        elif op[0] == 'x27':
                new_val = random.choice([160])
                val[0] = new_val
                cond_str += op[-1]+', '
                cond_str += val[-1]
    ...
    def gen_inst(self,op_comb, val_comb, cgf):
        ...
        for op,val_soln in zip(op_comb,val_comb):
            ...
            if self.is_fext:
                ...
                print(self.xlen)
                if self.xlen == 32:
                    print("Combining Combinations for" + self.opcode )
                    instr_dict.append(self.__zcmp32_instr__(op,val))
                elif self.xlen == 64:
                    print("Combining Combinations for" + self.opcode )
                    instr_dict.append(self.__zcmp64_instr__(op,val))
            else:
